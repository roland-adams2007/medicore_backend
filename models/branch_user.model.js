...
//     try {
//       const [rows] = await db_connection.execute(
//         `SELECT bu.id, bu.branch_id, bu.user_id, bu.role_id, r.name AS role_name
//          FROM branch_users bu
//          JOIN roles r ON bu.role_id = r.id
//          WHERE bu.branch_id = ? AND bu.user_id = ?
//          LIMIT 1`,
...
//     try {
//       const {
//         staff_id, phone, alt_phone, gender, date_of_birth, profile_photo_url,
//         address, city, state_id, date_joined, date_left, employment_type,
//         salary, salary_frequency, specialization, license_number, license_expiry,
//         qualification, emergency_contact_name, emergency_contact_phone,
...
//         [
//           user_id, clinic_id,
//           staff_id || null, phone || null, alt_phone || null, gender || null,
//           date_of_birth || null, profile_photo_url || null, address || null,
//           city || null, state_id || null, date_joined || null, date_left || null,
//           employment_type || null, salary || null, salary_frequency || "monthly",
//           specialization || null, license_number || null, license_expiry || null,
//           qualification || null, emergency_contact_name || null,
...
//   findStaffByBranch: async ({
//     clinicId, branchId, search = null, roleId = null,
//     status = null, departmentId = null, limit = 20, offset = 0,
//   }) => {
//     try {
//       const params = [];
//       const conditions = ["bu.branch_id = ?", "b.clinic_id = ?"];
...
//       if (search) {
//         conditions.push(`(u.fname LIKE ? OR u.lname LIKE ? OR u.email LIKE ? OR sp.staff_id LIKE ?)`);
//         const like = `%${search}%`;
//         params.push(like, like, like, like);
//       }
//       if (roleId) { conditions.push("bu.role_id = ?"); params.push(roleId); }
//       if (status) { conditions.push("sp.status = ?"); params.push(status); }
//       if (departmentId) { conditions.push("sd.department_id = ?"); params.push(departmentId); }
...
//   invite: async ({ branch_id, clinic_id, email, role_id, invited_by, token, status, expires_at, created_at }) => {
//     try {
//       const [result] = await db_connection.execute(
//         `INSERT INTO branch_user_invites
//          (branch_id, clinic_id, email, role_id, invited_by, token, status, expires_at, created_at)
...
//   findStaffInviteByClinicAndBranch: async ({
//     branchId, clinicId, search = null, roleId = null,
...
//       if (search) {
//         conditions.push(`(bui.email LIKE ? OR u.fname LIKE ? OR u.lname LIKE ?)`);
//         const like = `%${search}%`;
//         params.push(like, like, like);
//       }
//       if (roleId) { conditions.push("bui.role_id = ?"); params.push(roleId); }
//       if (status) { conditions.push("bui.status = ?"); params.push(status); }
